...
txps <- select(txdb, keys(txdb, "TXID"), c("TXNAME","GENEID"), "TXID") |>
  tibble::as_tibble()
```
...
idx_nums <- txps |>
  filter(TXNAME %in% idx) |>
  arrange(match(TXNAME, idx)) |>
  pull(TXID)
e <- exons[idx_nums]
```
...
```{r}
library(plyranges)
# first one has negative estimate (KD / CTRL)
# this means it is the more common one when PTBP1 is around
# so the other ones possible contain SE with the C/U signal upstream
e[[1]] |>
  filter_by_non_overlaps(e[[2]])
e[[1]] |>
  filter_by_non_overlaps(e[[3]])
upstr <- e[[1]] |>
  filter_by_non_overlaps(e[[2]]) |>
  flank_upstream(100)
```
