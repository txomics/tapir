...
Obtain a GRanges of all the exons present in significant DTU. First, we obtain the GRangesList of all the significant transcripts. Then, we unlist the GRangesList into a GRanges with all the exons present in the significant transcripts.

```{r}
#| message: false
#| 
library(plyranges)
sig_transcripts <- exons[names(exons) %in% sig_res$isoform_id] # GRangesList 

# Efficiently annotate if exon is internal or not (needed later for exon-level detection)
sig_transcripts@unlistData$internal <- TRUE
sig_transcripts@unlistData$internal[start(sig_transcripts@partitioning)] <- FALSE
sig_transcripts@unlistData$internal[end(sig_transcripts@partitioning)] <- FALSE
  
flat_sig_exons <- unlist(sig_transcripts) #GRanges
flat_sig_exons
```
...
#include isoform ID for each transcript name 
...
flat_sig_exons <- flat_sig_exons |>
    dplyr::mutate(key = paste0(isoform, "-", exon_rank))
...
left_keys <- paste0(downreg_candidates$isoform, "-", 
                    downreg_candidates$exon_rank-1)
left_exons <- flat_sig_exons |>
  filter(key %in% left_keys)

right_keys <- paste0(downreg_candidates$isoform, "-", 
                     downreg_candidates$exon_rank+1)
